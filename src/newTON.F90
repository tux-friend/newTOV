...
! and efficient. 
! Feel free to commit and pull requests...
...
!----INPUT VALUES ------------------------------------------------------
	...
	itemp=0.5d0			! temperature (isothermal) in MeV
	rhoc=0.4893269d15	! central density in cgs
...
!-----------------------------------------------------------------------    
	...
	call betaeqYe(rhoc,itemp,iye,xprs,dmutot,xenr)
	...
	do while((Y(2).ge.0.0d0).and.(irho.ge.1.5d3)) 
		...
		call RK4(irad,irho,itemp,iye,xenr,dmutot,Y,h,TOV)
		irad=irad+h
	enddo
	close(1)
...
SUBROUTINE betaeqYe(rhoe,temp,yee,xprs,dmutot,xenr)
	...
	keytemp=1		! set EOSDrive input mode to: rho, T, Ye
	keyerr=0
	tol=1.0d-9
	i=0
	itermax=50
...
SUBROUTINE findrho(press,rho,xtemp,iye,xenr,dmutot)

	! find rho for given p, T, Ye with rudimentary bisection
	...
	do while(abs((xprs-press)/press).ge.tol.and.(j.le.itermax))     
		call nuc_eos_full(rho,xtemp,iye,xenr,xprs,xent,xcs2,xdedt,&
			xdpderho,xdpdrhoe,xxa,xxh,xxn,xxp,xabar,xzbar,xmu_e,xmu_n,&
			xmu_p,xmuhat,keytemp,keyerr,precision)
		call betaeqYe(rho,xtemp,iye,xprs,dmutot,xenr)
		call nuc_eos_full(rho,xtemp,iye,xenr,xprs,xent,xcs2,xdedt,&
			xdpderho,xdpdrhoe,xxa,xxh,xxn,xxp,xabar,xzbar,xmu_e,xmu_n,&
			xmu_p,xmuhat,keytemp,keyerr,precision)
...
SUBROUTINE RK4(irad,irho,itemp,iye,xenr,dmutot,Y,h,TOV)
	...
	DOUBLE PRECISION :: irad,irho,itemp,iye,h,dmutot,xenr

	iY=Y
	call stellarEq(irad,irho,itemp,iY,Y,TOV)
	k1=h*Y
	call findrho(iY(2)+k1(2)/2,irho,itemp,iye,xenr,dmutot)
	call stellarEq(irad+h/2,irho,itemp,iY+k1/2,Y,TOV)
	k2=h*Y
	call findrho(iY(2)+k2(2)/2,irho,itemp,iye,xenr,dmutot)
	call stellarEq(irad+h/2,irho,itemp,iY+k2/2,Y,TOV)
	k3=h*Y
	call findrho(iY(2)+k3(2)/2,irho,itemp,iye,xenr,dmutot)
	call stellarEq(irad+h,irho,itemp,iY+k3,Y,TOV)
	k4=h*Y
	Y=iY+(k1+2*k2+2*k3+k4)/6
	call findrho(Y(2),irho,itemp,iye,xenr,dmutot)

end SUBROUTINE RK4

SUBROUTINE stellarEq(irad,irho,itemp,iY,Y,TOV)
	...
	DOUBLE PRECISION itemp,xenr,iye,dmutot
    ...
    if(TOV.eq.0)then
		if(iY(1).eq.0.0d0)then
			...
			Y(2) = -ggrav*irho*8.d0/3*pi*irad
		else
			Y(1) = 4.d0*pi*irho*irad**2
			Y(2) = -ggrav*irho*iY(1)/irad**2
		endif
    else
		call betaeqYe(irho,itemp,iye,iY(2),dmutot,xenr)
